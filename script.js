...
function inicializa() {
  numeroDeCartas = prompt(
    "Com quantas cartas você quer jogar? (Número par de 4 à 14)"
  );
...
function distribuicaoDasCartas() {
  ...
  for (let i = 0; i < numeroDeCartas; i++) {
    if (
      (numeroDeCartas === 8 && i === 4) ||
      (numeroDeCartas === 10 && i === 5) ||
      (numeroDeCartas === 12 && i === 6) ||
      (numeroDeCartas === 14 && i === 7)
    ) {
      container.innerHTML += `
                <div class="break"></div>`;
    }
    container.innerHTML += `
            <div onclick="virar(this)" class="carta" data-test="card">
                <img src="./imagens/back.png" data-test="face-down-image" alt=""  >
                <img class="escondido" data-test="face-up-image" src="./${cartasEmJogo[i]}" alt="">
            </div>`;
  }
}
...
let alerta = function () {
  alert(
    `Você ganhou em ${jogadas} jogadas! A duração do jogo foi de ${segundos} segundos!`
  );
  clearInterval(cod);
...
function virar(elemento) {
  ...
  if (cartasViradas === 2) {
    if (
      elemento.children[1].getAttribute("src") !==
      cartaAnterior.children[1].getAttribute("src")
    ) {
